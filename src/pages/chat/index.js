...
export default function ChatPage() {
  ...
  const sendMessage = () => {
    ...
    sendSocketMessage({
      conversationId: selectedConv._1d || selectedConv._id,
      text: text.trim(),
    });
    setText("");
  };
  ...
  const otherMember = (() => {
    ...
    return (
      selectedConv.members.find((m) => {
        const id = typeof m === "string" ? m : m._id ? String(m._id) : null;
        return id !== String(me._id);
      }) ||
      (typeof selectedConv.members[0] === "string"
        ? null
        : selectedConv.members[0])
    );
  })();

  return (
    <div className="h-[calc(100vh-64px)] flex bg-gradient-to-br from-amber-100 via-emerald-50 to-emerald-200">
      {/* Left sidebar */}
      <aside className="w-80 bg-white rounded p-3 overflow-y-auto">
      ...
      <main className="flex-1 bg-white rounded p-4 flex flex-col">
        ...
        {selectedConv && (
          <>
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto mb-3 p-2">
